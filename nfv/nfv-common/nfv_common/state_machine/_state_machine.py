    ...
    def handle_event(self, event, event_data=None):
        ...
        try:
            ...
            if prev_state != self._current_state:
                ...
                DLOG.verbose(
                    "Nested handle event calls detected, "
            ...
            if next_state_name != prev_state.name:
                ...
                try:
                    ...
                    DLOG.error(
                        "Caught exception while trying to exit state "
                        "(%s), event=%s, error=%s." % (prev_state, event, e)
                    )
                    return

                # Attempt to transition from the current state
                try:
                    prev_state.transition(self._context, event, event_data, next_state)

                except StateException as e:
                    DLOG.error(
                        "Caught exception while trying to transition "
                        "from state (%s) to state (%s), event=%s, "
                        "error=%s." % (prev_state, next_state, event, e)
                    )
                    prev_state.enter(
                        self._context, *self._context_args, **self._context_kwargs
                    )
                    return
                ...
                try:
                    ...
                    for event_state, event, event_data in event_backlog:
                        if event_state != self._current_state:
                            DLOG.info(
                                "Ignoring event %s, no longer in state "
                                "%s, now in state %s."
                                % (event, event_state, self._current_state)
                            )
                        else:
                            DLOG.info(
                                "Handling event backlog, event=%s while "
                    ...
                    DLOG.error(
                        "Caught exception while trying to enter state "
                    ...
                    self._event_backlog_state = None
                    del self._event_backlog[:]
                    prev_state.transition(self._context, event, event_data, prev_state)
                    prev_state.enter(
                        self._context, *self._context_args, **self._context_kwargs
                    )
                    return
            ...
            DLOG.error(
                "Caught exception while trying to handle event (%s), "
                "error=%s." % (event, e)
            )
            return
