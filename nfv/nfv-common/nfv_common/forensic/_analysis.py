...
def _analysis_instances_success(
    instance_uuid,
    instance_name,
    records,
    expected_records,
    ...
    for record in records:
        ...
        if record_data["type"] == expected_records[idx]:
            if NFV_VIM.INSTANCE_NFVI_ACTION_START == record_data["type"]:
                ...
                NFV_VIM.INSTANCE_START_CALLBACK,
            ...
            elif record_data["type"] == NFV_VIM.INSTANCE_GUEST_SERVICES_ENABLE_CALLBACK:
                if record_data["instance_name"] == instance_name:
                    if "True" == record_data["completed"]:
                        if record_data["service_name"] == "heartbeat":
                            if record_data["service_admin_state"] == "unlocked":
                ...
                record_data["type"] == NFV_VIM.INSTANCE_GUEST_SERVICES_DISABLE_CALLBACK
            ):
                if record_data["instance_name"] == instance_name:
                    if "True" == record_data["completed"]:
                        if record_data["service_name"] == "heartbeat":
                ...
                NFV_VIM.INSTANCE_GUEST_SERVICES_VOTE,
                ...
                NFV_VIM.INSTANCE_GUEST_SERVICES_VOTE_CALLBACK,
...
def analysis_instance_start_success(
    ...
    return _analysis_instances_success(
        instance_uuid, instance_name, records, expected_records
    )
...
def analysis_instance_stop_success(
    ...
    return _analysis_instances_success(
        instance_uuid, instance_name, records, expected_records, action_types=["stop"]
    )
...
def analysis_instance_pause_success(
    ...
    return _analysis_instances_success(
        instance_uuid, instance_name, records, expected_records, action_types=["pause"]
    )
...
def analysis_instance_resume_success(
    ...
    return _analysis_instances_success(
        instance_uuid, instance_name, records, expected_records, action_types=["resume"]
    )
...
def analysis_instance_resize_success(
    ...
    return _analysis_instances_success(
        ...
        records,
        expected_records,
        action_types=["resize", "resize_begin", "resize_end"],
    )
...
def analysis_instance_resize_confirm_success(
    ...
    return _analysis_instances_success(
        ...
        records,
        expected_records,
        action_types=["confirm-resize", "resize", "resize_begin", "resize_end"],
    )
...
def analysis_instance_resize_revert_success(
    ...
    return _analysis_instances_success(
        ...
        records,
        expected_records,
        action_types=["revert-resize", "resize", "resize_begin", "resize_end"],
    )
...
def analysis_stdout(records):
    ...
    for idx, record in enumerate(records):
        ...
        if data["type"] in [
            ...
            if data["host_name"] in hosts:
                ...
                print("  %s  %s" % (timestamp_str(record["timestamp"]), data["log"]))

            hosts[data["host_name"]] = record

        elif data["type"] in [
            NFV_VIM.INSTANCE_DIRECTOR_EVACUATE_FAILED,
            ...
            NFV_VIM.INSTANCE_DIRECTOR_EVACUATE_SUCCESS,
            ...
            if data["instance_name"] in instances:
                ...
                print(
                    "    %s (%s=%s)  %s"
                    % (
                        timestamp_str(prev_record["timestamp"]),
                        "\u0394",
                        elapsed_time,
                        prev_record["data"]["log"],
                    )
                )
    ...
    hosts_state = sorted(
        list(hosts_state_change.values()), key=lambda k: k["timestamp"]
    )
    ...
    instances_state = sorted(
        list(instances_state_change.values()), key=lambda k: k["timestamp"]
    )
    ...
    for idx, instance in enumerate(instances_state):
        ...
        print(
            "%4i. %s %32s (%s): %s %s %s (%s %s %s) on host %s"
            % (
                idx,
                timestamp_str(timestamp),
