    ...
    def selobj_error_callback(self):
        while True:
            ...
            if client_connection is not None:
                ...
                DLOG.info(
                    "Client connection error, from %s, port=%s."
    ...
    def dispatch(self):
        while True:
            ...
            if select_obj == self._connection.selobj:
                ...
                client_connection = TCPConnection(
                    client_ip, client_port, client_socket, False, self, self._auth_key
                )
                selobj.selobj_add_read_obj(client_connection.selobj, self.dispatch)
                selobj.selobj_add_error_callback(
                    client_connection.selobj, self.selobj_error_callback
                )
                self._client_connections[client_connection.selobj] = client_connection

                DLOG.verbose(
                    "Client connected from %s, port=%s." % (client_ip, client_port)
                )
            else:
                # Client Data
                client_connection = self._client_connections.get(select_obj, None)
                if client_connection is not None:
                    ...
                    if msg is not None:
                        DLOG.verbose(
                            "Message received from %s, port=%s, "
                            "select_obj=%s."
