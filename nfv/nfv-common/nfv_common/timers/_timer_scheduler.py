...
class TimerScheduler:
    ...
    def schedule(self):
        ...
        if ms_expired < self._scheduler_interval_ms:
            DLOG.verbose(
                "Not enough time has elapsed to schedule timers, "
        ...
        if 0 != self._scheduler_timestamp_ms:
            if ms_expired >= self._scheduler_max_delay_ms:
                if self._scheduling_on_time:
                    ...
                    DLOG.info("Not scheduling on time, elapsed=%d ms." % ms_expired)

                self._scheduler_delay_timestamp_ms = get_monotonic_timestamp_in_ms()
            else:
        ...
        try:
            ...
            for timer in self._timers:
                ...
                histogram.add_histogram_data(
                    "timer callback: " + timer.timer_name,
                    elapsed_ms // 100,
                    "decisecond",
                )
            ...
            histogram.add_histogram_data(
                "timer overall time per dispatch: ", elapsed_ms // 100, "decisecond"
            )
    ...
    def delete_timer(self, timer_id):
        ...
        if self._scheduling_timers:
            ...
            self._timers[:] = [
                timer for timer in self._timers if timer_id != timer.timer_id
            ]
