#
# Copyright (c) 2020-2023, 2025-2026 Wind River Systems, Inc.
#
...
class TestBuildStrategy(sw_update_testcase.SwUpdateStrategyTestCase):
    ...
    def _create_kube_rootca_update_strategy(
        self,
        sw_update_obj,
...
class ApplyStageMixin(object):
    ...
    def _create_built_kube_rootca_update_strategy(
        self,
        sw_update_obj,
        single_controller=False,
    ...
    def _kube_rootca_update_start_stage(self):
        return {
            ...
            "total_steps": 1,
            "steps": [
                {"name": "kube-rootca-update-start", "success_state": "update-started"},
            ],
        }
    ...
    def _kube_rootca_update_host_stages(self, stage_method, host_list):
        stages = []
        if host_list:
    ...
    def _kube_rootca_update_host_trustbothcas_stages(self, host_list=None):
        return self._kube_rootca_update_host_stages(
            self._kube_rootca_update_host_trustbothcas_stage, host_list
        )

    def _kube_rootca_update_host_trustnewca_stages(self, host_list=None):
        return self._kube_rootca_update_host_stages(
            self._kube_rootca_update_host_trustnewca_stage, host_list
        )

    def _kube_rootca_update_host_updatecerts_stages(self, host_list=None):
        return self._kube_rootca_update_host_stages(
            self._kube_rootca_update_host_updatecerts_stage, host_list
        )
    ...
    def build_stage_list(
        ...
        if add_host_trustnew:
            stages.extend(self._kube_rootca_update_host_trustnewca_stages(self.hosts()))
