        ...
        for x in range(1, 200):
            ...
            while True:
                ...
                if (old_state == L3_REBALANCE_STATE.GET_ROUTERS_HOSTED_ON_AGENT) and (
                    new_state == L3_REBALANCE_STATE.GET_ROUTER_PORT_NETWORKS
                ):
                ...
                if (_L3Rebalance.get_state() == L3_REBALANCE_STATE.DONE) and (
                    ...
                    if len(_L3Rebalance.num_routers_on_agents) > 2:
                        num_routers_length = len(_L3Rebalance.num_routers_on_agents)
                        assert (
                            num_routers_length == 0
                        ) or _L3Rebalance.num_routers_on_agents[0] == 0
    ...
    def test_rebalance_up_host_randomized_w_api_calls(self):
        ...
        for x in range(1, 200):
            ...
            while True:
                ...
                if (old_state == L3_REBALANCE_STATE.GET_ROUTERS_HOSTED_ON_AGENT) and (
                    (new_state == L3_REBALANCE_STATE.GET_ROUTER_PORT_NETWORKS)
                    or (new_state == L3_REBALANCE_STATE.DONE)
                ):
                ...
                if (_L3Rebalance.get_state() == L3_REBALANCE_STATE.DONE) and (
                    ...
                    if len(_L3Rebalance.num_routers_on_agents) > 2:
                        assert initial_router_count == final_router_count
                        assert (
                            max(_L3Rebalance.num_routers_on_agents)
            ...
            while True:
                ...
                if doing_abort:
                    ...
                    if (old_state != L3_REBALANCE_STATE.DONE) and (
                        ...
                        assert new_state == L3_REBALANCE_STATE.HOLD_OFF
                ...
                if (
                    (_L3Rebalance.get_state() == L3_REBALANCE_STATE.DONE)
                    and (len(_L3Rebalance.host_up_queue) == 0)
                    and (len(_L3Rebalance.host_down_queue) == 0)
                ):
                    final_router_count = sum(_L3Rebalance.num_routers_on_agents)
                    if DEBUG_PRINTING:
                        print(
