...
class TestFwUpdateStrategy(sw_update_testcase.SwUpdateStrategyTestCase):
    ...
    def test_fw_update_strategy_worker_stages_ignore(self):
        ...
        success, reason = strategy._add_worker_strategy_stages(
            sorted(worker_hosts, key=lambda host: host.name), reboot=True
        )
        ...
        expected_results = {"completion_percentage": 100, "total_stages": 0}
    ...
    def test_fw_update_strategy_storage_serial_no_instances(self):
        ...
        strategy._add_worker_strategy_stages(
            sorted(fw_update_host_list, key=lambda host: host.name), reboot=True
        )
    ...
    def test_fw_update_strategy_standard_parallel_migrate_host_aggregate(self):
        ...
        self.create_host_aggregate(
            "aggregate-1", ["compute-0", "compute-2", "compute-4", "compute-6"]
        )

        self.create_host_aggregate(
            "aggregate-2", ["compute-1", "compute-3", "compute-5", "compute-7"]
        )
    ...
    def test_fw_update_strategy_aio_sx(self):
        ...
        strategy = create_fw_update_strategy(
            worker_apply_type=SW_UPDATE_APPLY_TYPE.SERIAL, single_controller=True
        )
            ...
            "stages": [
                {
                    ...
                    "steps": [
                        {"name": "query-alarms"},
                        {"name": "fw-update-hosts", "entity_names": ["controller-0"]},
        ...
        expected_results = {
            ...
            "stages": [
                ...
                },
                {
                    ...
                    "steps": [
                        {"name": "query-alarms"},
                        {"name": "fw-update-hosts", "entity_names": ["controller-1"]},
    ...
    def test_fw_update_strategy_aio_dx_plus_parallel_locked_hosts(self):
        ...
        strategy._add_worker_strategy_stages(
            sorted(fw_update_worker_host_list, key=lambda host: host.name), reboot=True
        )
