...
class TestNFVClientShell(testcase.NFVTestCase):
    ...
    def _test_shell_bad_or_empty_args(
        self, mock_usage=None, mock_message=None, shell_args=None
    ):
...
class StrategyMixin:
    ...
    @mock.patch("nfv_client.sw_update.show_strategy")
    def _test_shell_show_incomplete_env(
        self, mock_show=None, shell_args=None, pop_env=None, expect_fails=True
    ):
    ...
    def test_shell_strategy_show_env_overrides(self):
        ...
        for env_val, override_val in self.MOCK_ENV_OVERRIDES.items():
            ...
            self._test_shell_show_incomplete_env(
                shell_args=shell_args, pop_env=env_val, expect_fails=False
            )
