...
def process_progress_marker_start(marker_name):
    ...
    sys.stdout.write(
        "%s  %s %s " % (str(datetime.datetime.now())[:-3], marker_name, space_str)
    )
    sys.stdout.flush()
...
def process_do_setup(loads_dir, setup_data):
    ...
    directory = openstack.get_directory(config, openstack.SERVICE_CATEGORY.OPENSTACK)
    ...
    for resource in setup_data["resources"]:
        if "flavor" == resource["type"]:
            ...
            flavor = next((x for x in flavors if x["name"] == resource["name"]), None)
            if flavor is None:
                nova.create_flavor(
                    token,
                    resource["id"],
    ...
    for resource in setup_data["resources"]:
        if "image" == resource["type"]:
            ...
            if image is None:
                ...
                glance.upload_image_data_by_file(token, image_data["id"], image_file)
            process_progress_marker_end("[OKAY]")
    ...
    for resource in setup_data["resources"]:
        if "volume" == resource["type"]:
            ...
            volume = next((x for x in volumes if x["name"] == resource["name"]), None)
            if volume is None:
                if resource["image_name"] is not None:
                    image = next(
                        (x for x in images if x["name"] == resource["image_name"]), None
                    )
                    if image is None:
                        process_progress_marker_end("[FAILED]")
                        print(
                            "Image %s for volume %s does not exist."
    ...
    for resource in setup_data["resources"]:
        if "network" == resource["type"]:
            ...
            network = next((x for x in networks if x["name"] == resource["name"]), None)
            if network is None:
    ...
    for resource in setup_data["resources"]:
        if "subnet" == resource["type"]:
            ...
            subnet = next((x for x in subnets if x["name"] == resource["name"]), None)
            if subnet is None:
                network = next(
                    (x for x in networks if x["name"] == resource["network_name"]), None
                )
                if network is None:
                    process_progress_marker_end("[FAILED]")
                    print(
                        "Network %s for subnet %s does not exist."
    ...
    for resource in setup_data["resources"]:
        if "instance" == resource["type"]:
            ...
            server = next((x for x in servers if x["name"] == resource["name"]), None)
            if server is None:
                flavor = next(
                    (x for x in flavors if x["name"] == resource["flavor"]), None
                )
                if flavor is None:
                    process_progress_marker_end("[FAILED]")
                    print(
                        "Can't find flavor %s for instance %s"
                ...
                if resource["image"] is not None:
                    image = next(
                        (x for x in images if x["name"] == resource["image"]), None
                    )
                    if image is None:
                        process_progress_marker_end("[FAILED]")
                        print(
                            "Can't find image %s for instance %s"
                ...
                for network_name in resource["networks"]:
                    ...
                    if network is None:
                        process_progress_marker_end("[FAILED]")
                        print(
                            "Can't find network %s for instance %s"
                ...
                for attached_volume in resource["attached_volumes"]:
                    ...
                    nova.attach_volume(
                        token, server_id, volume["id"], attached_volume["device_name"]
                    )
...
def process_do_tests(test_data):
    ...
    for test in test_data["tests"]:
        if "pause_instance" == test["type"]:
            test = tests.TestInstancePause(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "unpause_instance" == test["type"]:
            test = tests.TestInstanceUnpause(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "suspend_instance" == test["type"]:
            test = tests.TestInstanceSuspend(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "resume_instance" == test["type"]:
            test = tests.TestInstanceResume(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "reboot_instance" == test["type"]:
            test = tests.TestInstanceReboot(
                test["entity"], test["timeout"], test["hard_reset"], test["guest_hb"]
            )

        elif "rebuild_instance" == test["type"]:
            test = tests.TestInstanceRebuild(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "stop_instance" == test["type"]:
            test = tests.TestInstanceStop(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "start_instance" == test["type"]:
            test = tests.TestInstanceStart(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "live_migrate_instance" == test["type"]:
            test = tests.TestInstanceLiveMigrate(
                test["entity"], test["timeout"], None, test["guest_hb"]
            )

        elif "cold_migrate_instance" == test["type"]:
            test = tests.TestInstanceColdMigrate(
                test["entity"], test["timeout"], None, test["guest_hb"]
            )

        elif "cold_migrate_confirm_instance" == test["type"]:
            test = tests.TestInstanceColdMigrateConfirm(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "cold_migrate_revert_instance" == test["type"]:
            test = tests.TestInstanceColdMigrateRevert(
                test["entity"], test["timeout"], test["guest_hb"]
            )

        elif "resize_instance" == test["type"]:
            test = tests.TestInstanceResize(
                test["entity"], test["flavors"], test["timeout"], test["guest_hb"]
            )

        elif "resize_confirm_instance" == test["type"]:
            test = tests.TestInstanceResizeConfirm(
                test["entity"], test["flavors"], test["timeout"], test["guest_hb"]
            )

        elif "resize_revert_instance" == test["type"]:
            test = tests.TestInstanceResizeRevert(
                test["entity"], test["flavors"], test["timeout"], test["guest_hb"]
            )

        elif "lock_host" == test["type"]:
            test = tests.TestHostLock(
                test["entity"], test["instances"], test["timeout"]
            )
    ...
    for test in test_set:
        ...
        if not test.setup():
            process_progress_marker_end(
                "[SETUP-FAILED]  (%s %s=%s)"
                % (
                    str(test.end_datetime)[:-3],
                    "\u0394",
        ...
        process_progress_marker_end(
            "[OKAY]  (%s %s=%s)"
            % (str(test.end_datetime)[:-3], "\u0394", str(test.elapsed_datetime)[:-3])
        )
    ...
    sys.stdout.write(
        "Total-Tests: %s     Execution-Time: %s\n"
...
def process_main():
    ...
    try:
        ...
        j2_env = Environment(
            loader=FileSystemLoader([setup_dir, tests_dir]), trim_blocks=True
        )
        ...
        arg_parser.add_argument(
            "-f", "--forever", help="loop forever doing tests", action="store_true"
        )
        arg_parser.add_argument(
            "-w", "--wipe", help="cleanup setup", action="store_true"
        )
        ...
        loop_i = 0
        while True:
            iteration_str = "===== Test Iteration %s (%s)" % (loop_i, max_iteration_str)
