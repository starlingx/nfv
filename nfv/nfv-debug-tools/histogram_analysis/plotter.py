        ...
        for i in range(len(procs)):
            ...
            with open(os.path.join(pth, csvFile), "rb") as f:
                ...
                for ts, at, h, n in reader:
                    ...
                    if (int(date) >= int(dateRange[0])) and (
                        int(date) <= int(dateRange[1])
                    ):
            ...
            graphData["trace" + str(i)] = go.Scatter(
                ...
                name=(
                    procs[i]
                    if not oneAxis
                    else (procs[i] + "_" + ("time" if execTime else "hits"))
                ),
            )
            if plots == 1:
...
if len(command) == 1:
    ...
    for i in range(1, len(command)):
        if command[i] == "-c":  # Use config file
            ...
            oneAxis = True
        elif (command[i] == "--help") or (
            command[i] == "--h"
        ):  # Print help message and exit script
            helpMessage()
            sys.exit()
...
if procs:
    ...
    warnings.append(
        "NO GRAPH GENERATED BECAUSE NO VALID GROUP OR PROCESS NAME SPECIFIED."
    )
