...
def nfvi_guest_services_create(instance_uuid, host_name, services, callback):
    ...
    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_create", instance_uuid, host_name, services, callback=callback
    )
    return cmd_id
...
def nfvi_guest_services_set(instance_uuid, host_name, services, callback):
    ...
    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_set", instance_uuid, host_name, services, callback=callback
    )
    return cmd_id
...
def nfvi_guest_services_delete(instance_uuid, callback):
    ...
    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_delete", instance_uuid, callback=callback
    )
    return cmd_id
...
def nfvi_guest_services_query(instance_uuid, callback):
    ...
    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_query", instance_uuid, callback=callback
    )
    return cmd_id
...
def nfvi_guest_services_vote(instance_uuid, host_name, action_type, callback):
    ...
    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_vote", instance_uuid, host_name, action_type, callback=callback
    )
    return cmd_id


def nfvi_guest_services_notify(
    instance_uuid, host_name, action_type, pre_notification, callback
):
    """Notify guest services."""

    cmd_id = _guest_plugin.invoke_plugin(
        "guest_services_notify",
        instance_uuid,
        host_name,
        action_type,
        pre_notification,
...
def nfvi_disable_guest_host_services(host_uuid, host_name, host_personality, callback):
...
def nfvi_enable_guest_host_services(host_uuid, host_name, host_personality, callback):
...
def nfvi_delete_guest_host_services(host_uuid, host_name, host_personality, callback):
    """Delete guest services."""
...
def nfvi_create_guest_host_services(host_uuid, host_name, host_personality, callback):
    """Create guest services."""

    cmd_id = _guest_plugin.invoke_plugin(
        "create_host_services",
        host_uuid,
        host_name,
        host_personality,
        callback=callback,
...
def nfvi_query_guest_host_services(host_uuid, host_name, host_personality, callback):
    """Query guest services."""
...
def nfvi_register_host_services_query_callback(callback):
    ...
    _guest_plugin.invoke_plugin(
        "register_host_services_query_callback", callback=callback
    )
...
def nfvi_register_guest_services_query_callback(callback):
    ...
    _guest_plugin.invoke_plugin(
        "register_guest_services_query_callback", callback=callback
    )
...
def nfvi_register_guest_services_state_notify_callback(callback):
    ...
    _guest_plugin.invoke_plugin(
        "register_guest_services_state_notify_callback", callback=callback
    )
...
def nfvi_register_guest_services_alarm_notify_callback(callback):
    ...
    _guest_plugin.invoke_plugin(
        "register_guest_services_alarm_notify_callback", callback=callback
    )
...
def nfvi_register_guest_services_action_notify_callback(callback):
    ...
    _guest_plugin.invoke_plugin(
        "register_guest_services_action_notify_callback", callback=callback
    )
