...
from nfv_vim.instance_fsm._instance_tasks import RebootTask

DLOG = debug.debug_get_logger("nfv_vim.state_machine.instance")


class RebootState(state_machine.State):
    """
    Instance - Reboot State
    """

    def __init__(self, name):
        super(RebootState, self).__init__(name)

    def enter(self, instance):
        """
        Entering reboot state
        """
        DLOG.info("Entering state (%s) for %s." % (self.name, instance.name))
        instance.task = RebootTask(instance)
        instance.task.start()

    def exit(self, instance):
        """
        Exiting reboot state
        """
        DLOG.info("Exiting state (%s) for %s." % (self.name, instance.name))
        if isinstance(instance.task, RebootTask):
            instance.task.abort()

    def transition(self, instance, event, event_data, to_state):
        """
        Transition from the reboot state
        """
        pass

    def handle_event(self, instance, event, event_data=None):
        """
        Handle event while in the reboot state
        """
        ...
        if not handled:
            if INSTANCE_EVENT.TASK_STOP == event:
                ...
                DLOG.debug("Reboot completed for %s." % instance.name)
                return INSTANCE_STATE.INITIAL

            elif INSTANCE_EVENT.TASK_FAILED == event:
                DLOG.info("Reboot failed for %s." % instance.name)
                instance.fail_action(instance.action_fsm_action_type, reason)
                return INSTANCE_STATE.INITIAL

            elif INSTANCE_EVENT.TASK_TIMEOUT == event:
                DLOG.info("Reboot timed out for %s." % instance.name)
