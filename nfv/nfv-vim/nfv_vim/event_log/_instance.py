...
_event_templates = {
    ...
    event_log.EVENT_ID.INSTANCE_COLD_MIGRATE_CONFIRM_REJECTED: {
        ...
        "importance": event_log.EVENT_IMPORTANCE.HIGH,
        "reason_text": (
            "Cold-Migrate-Confirm rejected for instance %(instance_name)s%(reason)s"
        ),
        "exclude_event_context": [],
        "event_context_data": {
            event_log.EVENT_CONTEXT.ADMIN: {
    ...
    event_log.EVENT_ID.INSTANCE_COLD_MIGRATE_REVERT_REJECTED: {
        ...
        "importance": event_log.EVENT_IMPORTANCE.HIGH,
        "reason_text": (
            "Cold-Migrate-Revert rejected for instance "
    ...
    event_log.EVENT_ID.INSTANCE_COLD_MIGRATE_REVERTED: {
        ...
        "event_type": event_log.EVENT_TYPE.ACTION_EVENT,
        "importance": event_log.EVENT_IMPORTANCE.HIGH,
        "reason_text": "Cold-Migrate-Revert complete for instance %(instance_name)s",
        "exclude_event_context": [],
        "event_context_data": {
            event_log.EVENT_CONTEXT.ADMIN: {
                "entity_type": "tenant.instance",
    ...
    event_log.EVENT_ID.INSTANCE_EVACUATE_REJECTED: {
        ...
        "event_type": event_log.EVENT_TYPE.ACTION_EVENT,
        "importance": event_log.EVENT_IMPORTANCE.HIGH,
        "reason_text": "Evacuate rejected for instance %(instance_name)s%(reason)s",
        "exclude_event_context": [],
        "event_context_data": {
            event_log.EVENT_CONTEXT.ADMIN: {
                "entity_type": "tenant.instance",
    ...
    event_log.EVENT_ID.INSTANCE_GUEST_HEARTBEAT_ESTABLISHED: {
        ...
        "reason_text": "Guest Heartbeat established for instance %(instance_name)s",
        "exclude_event_context": [],
        "event_context_data": {
            event_log.EVENT_CONTEXT.ADMIN: {
                "entity_type": "tenant.instance",
    ...
    event_log.EVENT_ID.INSTANCE_GUEST_HEARTBEAT_DISCONNECTED: {
        ...
        "event_type": event_log.EVENT_TYPE.ACTION_EVENT,
        "importance": event_log.EVENT_IMPORTANCE.MEDIUM,
        "reason_text": "Guest Heartbeat disconnected for instance %(instance_name)s",
        "exclude_event_context": [],
        "event_context_data": {
            event_log.EVENT_CONTEXT.ADMIN: {
                "entity_type": "tenant.instance",
...
def instance_issue_log(
    ...
    if additional_text is None:
        ...
        data["additional_text"] = str(additional_text).rstrip(". \t\n\r")
...
def instance_manage_events(instance, enabling=False):
    ...
    if event_id is not None and not last_event(event_id):
        events = instance_issue_log(
            instance, event_id, additional_text=additional_text, reason=reason
        )
    ...
    if instance.is_rebooting():
        ...
        if last_event(event_log.EVENT_ID.INSTANCE_LIVE_MIGRATING):
            ...
            if (
                instance.from_instance_type_original_name
            ...
            if not (
                last_event(event_log.EVENT_ID.INSTANCE_LIVE_MIGRATED)
                or last_event(event_log.EVENT_ID.INSTANCE_LIVE_MIGRATE_FAILED)
                or last_event(event_log.EVENT_ID.INSTANCE_COLD_MIGRATED)
                or last_event(event_log.EVENT_ID.INSTANCE_COLD_MIGRATE_FAILED)
                or last_event(event_log.EVENT_ID.INSTANCE_COLD_MIGRATE_REVERTED)
                or last_event(event_log.EVENT_ID.INSTANCE_RESIZE_REVERTED)
            ):
