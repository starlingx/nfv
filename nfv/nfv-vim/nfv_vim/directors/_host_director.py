...
class HostDirector(metaclass=Singleton):
    ...
    def _nfvi_lock_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Lock of host %s failed, reason=%s."
    ...
    def _nfvi_disable_host_services_callback(self, service):
        ...
        if not response["completed"]:
            DLOG.info(
                "Disable of %s services on host %s failed"
                ", reason=%s." % (service, response["host_name"], response["reason"])
            )
            ...
            if (
                OPERATION_TYPE.DISABLE_HOST_SERVICES
    ...
    def _nfvi_disable_host_services(
        ...
        if service == objects.HOST_SERVICES.COMPUTE:
            ...
            nfvi.nfvi_disable_guest_host_services(
                ...
                host_name,
                host_personality,
                self._nfvi_disable_host_services_callback(objects.HOST_SERVICES.GUEST),
            )
    ...
    def _nfvi_enable_host_services_callback(self, service):
        ...
        if not response["completed"]:
            DLOG.info(
                "Enable of %s services on host %s failed, reason=%s."
            ...
            if (
                OPERATION_TYPE.ENABLE_HOST_SERVICES
                != self._host_operation.operation_type
            ):
                DLOG.verbose(
                    "Unexpected host %s operation %s, ignoring."
    ...
    def _nfvi_enable_host_services(
        ...
        if service == objects.HOST_SERVICES.COMPUTE:
            nfvi.nfvi_enable_compute_host_services(
                ...
                host_name,
                host_personality,
                self._nfvi_enable_host_services_callback(objects.HOST_SERVICES.COMPUTE),
            )
        elif service == objects.HOST_SERVICES.GUEST:
            nfvi.nfvi_enable_guest_host_services(
                ...
                host_name,
                host_personality,
                self._nfvi_enable_host_services_callback(objects.HOST_SERVICES.GUEST),
            )
        elif service == objects.HOST_SERVICES.CONTAINER:
            ...
            nfvi.nfvi_enable_network_host_services(
                ...
                host_name,
                host_personality,
                self._nfvi_enable_host_services_callback(objects.HOST_SERVICES.NETWORK),
            )
        else:
    ...
    def _nfvi_unlock_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Unlock of host %s failed, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_reboot_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Reboot of host %s failed, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_upgrade_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Upgrade of host %s failed, reason=%s."
                % (
                    response["host_name"],
                    response.get("error-message", response["reason"]),
                )
            )
            ...
            DLOG.info(
                "Upgrade of host %s succeeded, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_swact_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Swact of host %s failed, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_fw_update_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Firmware Image Update for host %s failed, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_fw_update_abort_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Get Host Devices for host %s failed, reason=%s."
                % (response["host_name"], response["reason"])
            )
            ...
            if (
                OPERATION_TYPE.FW_UPDATE_ABORT_HOSTS
                != self._host_operation.operation_type
            ):
                DLOG.verbose(
                    "Unexpected host %s operation %s, ignoring."
    ...
    def host_audit(host):
        ...
        DLOG.verbose(
            "Notify other directors that a host %s audit is in progress." % host.name
        )
        instance_director = directors.get_instance_director()
    ...
    def host_abort(host):
        ...
        DLOG.info(
            "Notify other directors that a host %s abort is in progress." % host.name
        )
        instance_director = directors.get_instance_director()
    ...
    def lock_hosts(self, host_names):
        ...
        if self._host_operation is not None:
            DLOG.debug(
                ...
                % (self._host_operation.operation_type, host_operation.operation_type)
            )
            self._host_operation = None
    ...
    def _nfvi_kube_host_upgrade_control_plane_callback(self):
        ...
        response = yield
        DLOG.verbose("NFVI Kube Host Upgrade Control Plane response=%s." % response)
        if not response["completed"]:
            DLOG.info(
                "Kube Host Upgrade Control Plane failed. Host:%s, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def kube_upgrade_hosts_control_plane(self, host_names, force):
        ...
        for host_name in host_names:
            ...
            host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
            self._nfvi_kube_host_upgrade_control_plane(host.uuid, host.name, force)
    ...
    def _nfvi_kube_host_cordon_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Kube Host Upgrade Cordon failed. Host:%s, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def kube_host_cordon(self, host_names, force):
        ...
        for host_name in host_names:
            ...
            host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
            self._nfvi_kube_host_cordon(host.uuid, host.name, force)
        if host_operation.is_inprogress():
            self._host_operation = host_operation
        return host_operation
    ...
    def _nfvi_kube_host_uncordon_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Kube Host Upgrade Uncordon failed. Host:%s, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def kube_host_uncordon(self, host_names, force):
        ...
        for host_name in host_names:
            ...
            host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
            self._nfvi_kube_host_uncordon(host.uuid, host.name, force)
        if host_operation.is_inprogress():
            self._host_operation = host_operation
        return host_operation
    ...
    def _nfvi_kube_host_upgrade_kubelet_callback(self):
        ...
        response = yield
        DLOG.verbose("NFVI Kube Host Upgrade Kubelet response=%s." % response)
        if not response["completed"]:
            DLOG.info(
                "Kube Host Upgrade Kubelet failed. Host:%s, reason=%s."
                % (response["host_name"], response["reason"])
            )
    ...
    def _nfvi_kube_rootca_update_host_callback(self):
        ...
        if not response["completed"]:
            DLOG.info(
                "Kube Root CA Update failed. Host:%s, reason=%s."
                % (response["host_name"], response["reason"])
            )
            ...
            if (
                OPERATION_TYPE.KUBE_ROOTCA_UPDATE_HOSTS
                != self._host_operation.operation_type
            ):
                DLOG.verbose(
                    "Unexpected host %s operation %s, ignoring."
    ...
    def _nfvi_kube_rootca_update_host(
        self,
        host_uuid,
        host_name,
        update_type,
    ...
    def kube_rootca_update_hosts_by_type(
        ...
        for host_name in host_names:
            ...
            if host is None:
                ...
                return host_operation
            host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
            self._nfvi_kube_rootca_update_host(
                host.uuid,
                host.name,
                update_type,
                in_progress_state,
    ...
    def disable_host_services(self, host_names, service):
        ...
        for host_name in host_names:
            ...
            if objects.HOST_SERVICE_STATE.DISABLED == host.host_service_state(service):
                host_operation.add_host(host.name, OPERATION_STATE.COMPLETED)
            else:
                host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
        ...
        for host in host_list:
            self._nfvi_disable_host_services(
                host.uuid, host.name, host.personality, host.is_offline, service
            )
    ...
    def enable_host_services(self, host_names, service):
        ...
        for host_name in host_names:
            ...
            if objects.HOST_SERVICE_STATE.ENABLED == host.host_service_state(service):
                host_operation.add_host(host.name, OPERATION_STATE.COMPLETED)
            else:
                host_operation.add_host(host.name, OPERATION_STATE.INPROGRESS)
                host_list.append(host)
