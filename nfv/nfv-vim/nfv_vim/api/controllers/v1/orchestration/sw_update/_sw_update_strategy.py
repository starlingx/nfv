...
class SwUpdateStrategyStageStepData(wsme_types.Base):
    ...
    entity_type = wsme_types.wsattr(str, name="entity-type")
    entity_uuids = wsme_types.wsattr([str], name="entity-uuids")
    entity_names = wsme_types.wsattr([str], name="entity-names")
    result = wsme_types.wsattr(str, name="result")
    reason = wsme_types.wsattr(str, name="reason")
    start_date_time = wsme_types.wsattr(str, name="start-date-time")
    end_date_time = wsme_types.wsattr(str, name="end-date-time")
...
class SwUpdateStrategyPhaseData(wsme_types.Base):
    ...
    result = wsme_types.wsattr(str, name="result")
    reason = wsme_types.wsattr(str, name="reason")
    response = wsme_types.wsattr(str, name="response")
    start_date_time = wsme_types.wsattr(str, name="start-date-time")
    end_date_time = wsme_types.wsattr(str, name="end-date-time")
...
class SwUpdateStrategyData(wsme_types.Base):
    ...
    storage_apply_type = wsme_types.wsattr(
        SwUpdateApplyTypes, name="storage-apply-type"
    )
    swift_apply_type = wsme_types.wsattr(SwUpdateApplyTypes, name="swift-apply-type")
    worker_apply_type = wsme_types.wsattr(SwUpdateApplyTypes, name="worker-apply-type")
    max_parallel_worker_hosts = wsme_types.wsattr(int, name="max-parallel-worker-hosts")
    default_instance_action = wsme_types.wsattr(
        SwUpdateInstanceActionTypes, name="default-instance-action"
    )
    alarm_restrictions = wsme_types.wsattr(
        SwUpdateAlarmRestrictionTypes, name="alarm-restrictions"
    )
    state = wsme_types.wsattr(str, name="state")
    current_phase = wsme_types.wsattr(str, name="current-phase")
    current_phase_completion_percentage = wsme_types.wsattr(
        int, name="current-phase-completion-percentage"
    )
...
class SwUpgradeStrategyCreateData(wsme_types.Base):
    ...
    worker_apply_type = wsme_types.wsattr(
        SwUpdateApplyTypes, mandatory=True, name="worker-apply-type"
    )
    max_parallel_worker_hosts = wsme_types.wsattr(
        int, mandatory=False, name="max-parallel-worker-hosts"
    )
    default_instance_action = wsme_types.wsattr(
        SwUpdateInstanceActionTypes, mandatory=True, name="default-instance-action"
    )
    alarm_restrictions = wsme_types.wsattr(
        SwUpdateAlarmRestrictionTypes,
        mandatory=False,
...
class SwUpdateStrategyQueryData(wsme_types.Base):
    ...
    def convert_strategy_phase(phase_data):
        ...
        for stage_data in phase_data["stages"]:
            ...
            for step_data in stage_data["steps"]:
                ...
                # entity_type, entity_uuids and entity_names are not available in
                # the software system-deploy show response
                step.entity_type = step_data.get("entity_type")
                step.entity_uuids = step_data.get("entity_uuids")
                step.entity_names = step_data.get("entity_names")
    ...
    def convert_strategy(self, strategy_data):
        ...
        strategy.max_parallel_worker_hosts = strategy_data["max_parallel_worker_hosts"]
...
class SwUpdateStrategyActionAPI(rest.RestController):
    ...
    def post(self, request_data):
        if wsme_types.Unset == request_data.stage_id:
            if (
                ...
                or SW_UPDATE_ACTION.ABORT_STAGE == request_data.action
            ):
        ...
        if (
            SW_UPDATE_ACTION.APPLY_ALL == request_data.action
            or SW_UPDATE_ACTION.APPLY_STAGE == request_data.action
        ):
            ...
            SW_UPDATE_ACTION.ABORT == request_data.action
            or SW_UPDATE_ACTION.ABORT_STAGE == request_data.action
        ):
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "post":
            policy.check(
                sw_update_strategy_policy.POLICY_ROOT % "post",
                {},
                auth_context_dict,
...
class SwUpdateStrategyAPI(rest.RestController):
    ...
    @wsme_pecan.wsexpose(SwUpdateStrategyQueryData, str, status_code=httplib.OK)
    def get_one(self, strategy_uuid):
        if not validate.valid_uuid_str(strategy_uuid):
            ...
            return pecan.abort(
                httplib.BAD_REQUEST, "Invalid strategy uuid, uuid=%s" % strategy_uuid
            )
        ...
        if rpc.RPC_MSG_RESULT.SUCCESS == response.result:
            ...
            DLOG.debug("No strategy exists matching strategy uuid %s" % strategy_uuid)
            return pecan.abort(httplib.NOT_FOUND)
    ...
    def get_all(self):
        rpc_request = rpc.APIRequestGetSwUpdateStrategy()
        rpc_request.sw_update_type = _get_sw_update_type_from_path(pecan.request.path)
        vim_connection = pecan.request.vim.open_connection()
        vim_connection.send(rpc_request.serialize())
        msg = vim_connection.receive(timeout_in_secs=30)
        if msg is None:
    ...
    @wsme_pecan.wsexpose(
        None, str, body=SwUpdateStrategyDeleteData, status_code=httplib.OK
    )
...
class SwUpgradeStrategyAPI(SwUpdateStrategyAPI):
    ...
    def post(self, request_data):
        ...
        if wsme_types.Unset != request_data.max_parallel_worker_hosts:
            if (
                request_data.max_parallel_worker_hosts < MIN_PARALLEL_HOSTS
                or request_data.max_parallel_worker_hosts > MAX_PARALLEL_UPGRADE_HOSTS
            ):
                return pecan.abort(
                    httplib.BAD_REQUEST, "Invalid value for max-parallel-worker-hosts"
                )
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "delete":
            ...
            policy.check(
                sw_upgrade_strategy_policy.POLICY_ROOT % "get",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
            ...
            policy.check(
                sw_upgrade_strategy_policy.POLICY_ROOT % "add",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
...
class SystemConfigUpdateStrategyAPI(SwUpdateStrategyAPI):
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "delete":
            ...
            policy.check(
                system_config_update_strategy_policy.POLICY_ROOT % "get",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
            ...
            policy.check(
                system_config_update_strategy_policy.POLICY_ROOT % "add",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
...
class FwUpdateStrategyAPI(SwUpdateStrategyAPI):
    ...
    def post(self, request_data):
        ...
        if wsme_types.Unset != request_data.max_parallel_worker_hosts:
            if (
                request_data.max_parallel_worker_hosts < MIN_PARALLEL_HOSTS
                or request_data.max_parallel_worker_hosts > MAX_PARALLEL_FW_UPDATE_HOSTS
            ):
                return pecan.abort(
                    httplib.BAD_REQUEST, "Invalid value for max-parallel-worker-hosts"
                )
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "delete":
            ...
            policy.check(
                fw_update_strategy_policy.POLICY_ROOT % "get",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
            ...
            policy.check(
                fw_update_strategy_policy.POLICY_ROOT % "add",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
...
class KubeRootcaUpdateStrategyAPI(SwUpdateStrategyAPI):
    ...
    def post(self, request_data):
        ...
        if wsme_types.Unset != request_data.expiry_date:
            ...
            is_valid, reason = validate.validate_expiry_date(request_data.expiry_date)
            if not is_valid:
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "delete":
            ...
            policy.check(
                kube_rootca_update_strategy_policy.POLICY_ROOT % "get",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
            ...
            policy.check(
                kube_rootca_update_strategy_policy.POLICY_ROOT % "add",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
...
class KubeUpgradeStrategyAPI(SwUpdateStrategyAPI):
    ...
    def post(self, request_data):
        ...
        if wsme_types.Unset != request_data.max_parallel_worker_hosts:
            ...
            if request_data.max_parallel_worker_hosts > MAX_PARALLEL_KUBE_UPGRADE_HOSTS:
                return pecan.abort(
                    httplib.BAD_REQUEST,
                    "Invalid value for max-parallel-worker-hosts:(%s) > (%s)"
                    % (
    ...
    def enforce_policy(self, method_name, auth_context_dict):
        ...
        if method_name == "delete":
            ...
            policy.check(
                kube_upgrade_strategy_policy.POLICY_ROOT % "get",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
            ...
            policy.check(
                kube_upgrade_strategy_policy.POLICY_ROOT % "add",
                {},
                auth_context_dict,
                exc=policy.PolicyForbidden,
            )
