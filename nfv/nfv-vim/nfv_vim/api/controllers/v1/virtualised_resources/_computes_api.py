...
ComputeOperationType = wsme_types.Enum(
    str, "start", "stop", "pause", "unpause", "suspend", "resume", "reboot"
)
...
class ComputeOperateAPI(pecan.rest.RestController):
    ...
    def _do_operation(rpc_request):
        ...
        DLOG.error(
            "Unexpected result received for %s, result=%s."
...
class ComputeMigrateAPI(pecan.rest.RestController):
    ...
    def post(self, compute_id, request_data):
        ...
        if httplib.ACCEPTED != http_response:
            DLOG.error(
                "Compute migrate %s failed for %s, http_response=%s."
...
class ComputeCreateVirtualCpuType(wsme_types.Base):
    ...
    virtual_cpu_oversubscription_policy = wsme_types.wsattr(str, mandatory=False)
    virtual_cpu_pinning = wsme_types.wsattr(
        ComputeCreateVirtualCpuPinningType, mandatory=False
    )
...
class ComputeCreateFlavourType(wsme_types.Base):
    ...
    virtual_memory = wsme_types.wsattr(ComputeCreateVirtualMemoryType, mandatory=True)
    virtual_storage = wsme_types.wsattr(ComputeCreateVirtualStorageType, mandatory=True)
...
class ComputesAPI(pecan.rest.RestController):
    ...
    def _get_compute_details(compute_id, compute):
        ...
        DLOG.error(
            "Unexpected result received for compute %s, result=%s."
    ...
    @wsme_pecan.wsexpose(
        ComputeQueryData, body=ComputeCreateData, status_code=httplib.CREATED
    )
    def post(self, compute_create_data):
        DLOG.verbose(
            "Compute-API create called for compute %s." % compute_create_data.compute_id
        )
        ...
        rpc_request.live_migration_timeout = meta_data.get(
            "hw:wrs:live_migration_timeout", None
        )
        rpc_request.live_migration_max_downtime = meta_data.get(
            "hw:wrs:live_migration_max_downtime", None
        )
        vim_connection.send(rpc_request.serialize())
        msg = vim_connection.receive()
        if msg is None:
            DLOG.error(
                "No response received for compute %s." % compute_create_data.compute_id
            )
            return pecan.abort(httplib.INTERNAL_SERVER_ERROR)
    ...
    def delete(self, compute_id):
        ...
        DLOG.error(
            "Unexpected result received for instance %s, result=%s."
            % (compute_id, response.result)
        )
        return pecan.abort(httplib.INTERNAL_SERVER_ERROR)
