...
def _nfvi_host_query_callback():
    ...
    if response["completed"]:
        ...
        DLOG.error("Query-Host callback, not completed, responses=%s." % response)
...
def _nfvi_host_add_callback(nfvi_host_uuid, nfvi_host_name):
    ...
    if host is None:
        nfvi.nfvi_get_host(nfvi_host_uuid, nfvi_host_name, _nfvi_host_query_callback())
    else:
        host.nfvi_host_add()
...
def _nfvi_host_action_callback(nfvi_host_uuid, nfvi_host_name, do_action):
    ...
    if host is not None:
        if nfvi.objects.v1.HOST_ACTION.UNLOCK == do_action:
            ...
            DLOG.info(
                "Unknown action %s received for %s." % (do_action, nfvi_host_name)
            )

    return True


def _nfvi_host_state_change_callback(
    nfvi_host_uuid,
    nfvi_host_name,
    nfvi_admin_state,
    nfvi_oper_state,
    ...
    host = host_table.get(nfvi_host_name, None)
    if host is not None:
        host.nfvi_host_state_change(
            nfvi_admin_state, nfvi_oper_state, nfvi_avail_status, nfvi_data
        )
    return True
...
def _nfvi_sw_update_get_callback():
    ...
    DLOG.verbose(
        "Software update type=%s, in_progress=%s." % (sw_update_type, in_progress)
    )
...
def _nfvi_host_get_callback(nfvi_host_uuid, nfvi_host_name):
    ...
    DLOG.info(
        "Host %s has %s instances, failed=%s, stopped=%s."
...
def _nfvi_host_notification_callback(host_ip, nfvi_notify_type, nfvi_notify_data):
    ...
    if nfvi.objects.v1.HOST_NOTIFICATIONS.BOOTING == nfvi_notify_type:
        ...
        if host_name is not None:
            ...
            for instance in instance_table.on_host(host_name):
                if not (
                    ...
                    or instance.is_rebuilding()
                    or instance.is_migrating()
        ...
        DLOG.error(
            "Unknown host notification received, type=%s, data=%s"
...
def _query_nfvi_instance_callback():
    ...
    if response["completed"]:
        ...
        DLOG.error("Query-Instance callback, not completed, responses=%s." % response)
...
def _nfvi_instance_state_change_callback(nfvi_instance):
    ...
    if instance is None:
        if (
            ...
            nfvi.nfvi_get_instance(nfvi_instance.uuid, _query_nfvi_instance_callback())
    else:
        ...
        if nfvi_instance.recovery_priority is None:
            nfvi_instance.recovery_priority = instance._nfvi_instance.recovery_priority
        ...
        if nfvi.objects.v1.INSTANCE_AVAIL_STATUS.DELETED in nfvi_instance.avail_status:
...
def _nfvi_instance_action_change_callback(
    ...
    if instance is not None:
        instance.nfvi_instance_action_change(
...
def _nfvi_guest_services_alarm_notify_callback(
    ...
    if nfvi.objects.v1.INSTANCE_AVAIL_STATUS.FAILED == nfvi_avail_status:
        ...
        if nfvi.objects.v1.INSTANCE_ACTION_TYPE.REBOOT == nfvi_repair_action:
            instance.guest_services_failed(
                do_soft_reboot=True, health_check_failed_only=True
            )
        elif nfvi.objects.v1.INSTANCE_ACTION_TYPE.STOP == nfvi_repair_action:
            instance.guest_services_failed(do_stop=True, health_check_failed_only=True)
        else:
...
def _nfvi_guest_services_action_notify_callback(
    ...
    instance.nfvi_instance_action_change(nfvi_action_type, nfvi_action_state, reason)
    return True
...
def vim_nfvi_events_initialize():
    ...
    nfvi.nfvi_register_host_state_change_callback(_nfvi_host_state_change_callback)
    ...
    nfvi.nfvi_register_sw_update_get_callback(_nfvi_sw_update_get_callback)
    ...
    if not nfvi.nfvi_compute_plugin_disabled():
        ...
        nfvi.nfvi_register_instance_action_callback(_nfvi_instance_action_callback)

        nfvi.nfvi_register_instance_delete_callback(_nfvi_instance_delete_callback)
