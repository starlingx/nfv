...
class NotifyHostEnabledTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Enabled callback for %s, response=%s."
                % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                ...
                if self.force_pass:
                    DLOG.info(
                        "Notify-Host-Enabled callback for %s, "
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Notify-Host-Enabled for %s %s." % (self._host.name, self._service)
        )
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            reason = (
                "Trying to notify host enabled for unknown "
...
class NotifyHostDisabledTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Disabled callback for %s, response=%s."
                % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                ...
                if self._service == objects.HOST_SERVICES.NETWORK:
                    DLOG.info(
                        "Queueing rebalance for host %s disable" % self._host.name
                    )
                ...
                if self.force_pass:
                    DLOG.info(
                        "Notify-Host-Disabled callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Notify-Host-Disabled for %s %s." % (self._host.name, self._service)
        )
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            reason = (
                "Trying to notify host disabled for unknown "
                "host service %s" % self._service
            )
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
...
class NotifyHostServicesDisableFailedTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Services-Disable-Failed callback "
            ...
            if response["completed"]:
                ...
                if self.force_pass:
                    DLOG.info(
                        "Notify-Host-Services-Disable-Failed callback "
                        "for %s, failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                    )
    ...
    def run(self):
        ...
        nfvi.nfvi_notify_host_services_disable_failed(
            self._host.uuid, self._host.name, self._host.reason, self._callback()
        )
        return state_machine.STATE_TASK_WORK_RESULT.WAIT, empty_reason
...
class NotifyHostServicesDeleteFailedTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Services-Delete-Failed callback "
                "for %s, response=%s." % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
...
class NotifyHostFailedTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Failed callback for %s, response=%s."
                % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                ...
                if self.force_pass:
                    DLOG.info(
                        "Notify-Host-Failed callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
...
class FailHostTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Fail-Host callback for %s, response=%s." % (self._host.name, response)
            )
            if not response["completed"]:
                ...
                if self.force_pass:
                    DLOG.info(
                        "Fail-Host callback for %s, failed, force-passing."
                        % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
    ...
    def run(self):
        ...
        if self._host.is_failed() and self._host.is_component_failure():
            DLOG.info(
                "Fail-Host for %s because of component failure." % self._host.name
            )

            host_director = directors.get_host_director()
            if not host_director.host_has_instances(self._host, skip_stopped=True):
                DLOG.info(
                    "Host %s does not have any instances, skip failing host."
                    % self._host.name
                )
                return state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason

            if self._host.is_offline():
                DLOG.info("Host %s is offline, skip failing host." % self._host.name)
                return state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
...
class CreateHostServicesTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Create-Host-Services callback for %s %s, "
                "response=%s." % (self._host.name, self._service, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                    state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                ...
                if self.force_pass:
                    DLOG.info(
                        "Create-Host-Services callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Create-Host-Services for %s %s." % (self._host.name, self._service)
        )
        ...
        if self._service == objects.HOST_SERVICES.GUEST:
            ...
            reason = "Trying to create unknown host service %s" % self._service
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
            return state_machine.STATE_TASK_WORK_RESULT.FAILED, reason
...
class DeleteHostServicesTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Delete-Host-Services callback for %s %s, "
                "response=%s." % (self._host.name, self._service, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                ...
                if self.force_pass:
                    DLOG.info(
                        "Delete-Host-Services callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                    ...
                    self._host.host_services_update(
                        self._service, objects.HOST_SERVICE_STATE.FAILED, reason
                    )
    ...
    def run(self):
        ...
        if host_director.host_has_instances(self._host):
            reason = (
                "Delete of host %s rejected because instances are "
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            reason = "Trying to delete unknown host service %s" % self._service
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
            return state_machine.STATE_TASK_WORK_RESULT.FAILED, reason
...
class WaitHostServicesCreatedTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Wait-Host-Services-Created callback for service: "
                "%s %s, response=%s." % (self._service, self._host.name, response)
            )

            if response["completed"]:
                if (
                    self._service == objects.HOST_SERVICES.NETWORK
                    and response["result-data"] != "enabled"
                ):
                    DLOG.verbose(
                        "Wait-Host-Services-Created callback for %s, "
                        "service %s failed" % (self._service, self._host.name)
                    )
                    return
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Wait-Host-Services-Created for %s for service %s."
    ...
    def handle_event(self, event, event_data=None):
        ...
        if HOST_EVENT.PERIODIC_TIMER == event:
            if not self._query_inprogress:
                DLOG.verbose(
                    "Wait-Host-Services-Created for %s for service "
                    "%s. Repeating query." % (self._host.name, self._service)
                )
                self._query_inprogress = True
                if self._service == objects.HOST_SERVICES.COMPUTE:
                    nfvi.nfvi_query_compute_host_services(
...
class EnableHostServicesTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Enable-Host-Services callback for service: %s %s, "
                "response=%s." % (self._service, self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                    state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                ...
                if self.force_pass:
                    DLOG.info(
                        "Enable-Host-Services callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                    ...
                    if self._service == objects.HOST_SERVICES.NETWORK:
                        DLOG.info(
                            "Queueing rebalance for host %s enable" % self._host.name
                        )
                        network_rebalance.add_rebalance_work_l3(self._host.name, False)
                        network_rebalance.add_rebalance_work_dhcp(
                            self._host.name, False
                        )
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Enable-Host-Services for %s for service %s."
            % (self._host.name, self._service)
        )
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            reason = "Trying to enable unknown host service %s" % self._service
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
            return state_machine.STATE_TASK_WORK_RESULT.FAILED, reason
...
class DisableHostServicesTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Disable-Host-Services callback for service: %s, %s, "
                "response=%s." % (self._service, self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
                ...
                if self.force_pass:
                    DLOG.info(
                        "Disable-Host-Services callback for %s, "
                        "failed, force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Disable-Host-Services for %s service %s."
            % (self._host.name, self._service)
        )
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            nfvi.nfvi_disable_container_host_services(
                ...
                self._host.name,
                self._host.personality,
                self._host.is_offline(),
                self._callback(),
            )
        else:
            reason = "Trying to disable unknown host service %s" % self._service
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
            return state_machine.STATE_TASK_WORK_RESULT.FAILED, reason
...
class WaitHostServicesDisabledTaskWork(state_machine.StateTaskWork):
    ...
    def timeout(self):
        ...
        if not self._host.has_reason():
            self._host.update_failure_reason(
                "Terminating pods on disabled host %s timed out." % self._host.name
            )
    ...
    def _get_callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Get-Terminating-Pods callback for service: "
                "%s %s, response=%s." % (self._service, self._host.name, response)
            )
            ...
            if response["completed"]:
                if response["result-data"] != "":
                    DLOG.info(
                        "Get-Terminating-Pods callback for %s, "
                        "pods %s terminating" % (self._service, response["result-data"])
                    )
                    return
                ...
                DLOG.info(
                    "Get-Terminating-Pods callback for %s, failed" % self._host.name
                )
    ...
    def _extend_callback(self):
        ...
        if response["completed"]:
            DLOG.info(
                "Extended host services disable timeout for host %s." % self._host.name
            )
            self._host.disable_extend_timestamp = timers.get_monotonic_timestamp_in_ms()
        else:
            DLOG.error(
                "Failed to extend host services disable timeout for "
    ...
    def run(self):
        ...
        DLOG.verbose(
            "Wait-Host-Services-Disabled for %s for service %s."
            % (self._host.name, self._service)
        )
        ...
        if HOST_EVENT.PERIODIC_TIMER == event:
            if self._service == objects.HOST_SERVICES.CONTAINER:
                if not self._query_inprogress:
                    ...
                    if 10 <= elapsed_secs:
                        DLOG.verbose(
                            "Wait-Host-Services-Disabled for %s for "
                            "service %s. Doing query."
                            % (self._host.name, self._service)
                        )
                        self._query_inprogress = True
                        nfvi.nfvi_get_terminating_pods(
                            self._host.name, self._get_callback()
                        )
            handled = True
...
class NotifyHostServicesEnabledTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Services-Enabled callback for %s, "
            ...
            if response["completed"]:
                ...
                if self.force_pass:
                    ...
                    self._host.host_services_update_all(
                        objects.HOST_SERVICE_STATE.FAILED, response.get("reason", None)
                    )
...
class NotifyHostServicesDisabledTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Services-Disabled callback for %s, "
                "response=%s." % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
...
class NotifyHostServicesDeletedTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "Notify-Host-Services-Deleted callback for %s, "
                "response=%s." % (self._host.name, response)
            )
            if response["completed"]:
                self.task.task_work_complete(
...
class AuditHostServicesTaskWork(state_machine.StateTaskWork):
    ...
    def _callback(self):
        ...
        if self.task is not None:
            DLOG.verbose(
                "query callback for service %s %s"
            ...
            if response["completed"]:
                ...
                if self.force_pass:
                    ...
                    DLOG.error(
                        "Audit-Host-Services callback for %s, %s"
                        "response=%s." % (self._host.name, self._service, response)
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.FAILED, response["reason"]
    ...
    def run(self):
        ...
        if self._service == objects.HOST_SERVICES.COMPUTE:
            ...
            reason = "Trying to query unknown host service %s" % self._service
            DLOG.error(reason)
            self._host.update_failure_reason(reason)
            return state_machine.STATE_TASK_WORK_RESULT.FAILED, reason
...
class NotifyInstancesHostDisablingTaskWork(state_machine.StateTaskWork):
    ...
    def __init__(self, task, host, force_pass=False):
        ...
        if config.section_exists("instance-configuration"):
            ...
            max_cold_migrate_wait_in_secs = int(
                section.get("max_cold_migrate_wait_in_secs", 900)
            )
            max_resize_wait_in_secs = int(section.get("max_resize_wait_in_secs", 900))
    ...
    def run(self):
        ...
        if self._host.is_failed() and not self._host.is_component_failure():
            DLOG.info(
                "Host %s is failed, skipping host disabling "
        ...
        if self._host.is_offline():
            DLOG.info(
                "Host %s is offline, skipping host disabling "
                "notifications to instances." % self._host.name
            )
            return state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
    ...
    def handle_event(self, event, event_data=None):
        ...
        if HOST_EVENT.INSTANCE_MOVED == event:
            ...
            DLOG.info("Host %s instance stopped extending timeout." % self._host.name)
            self.extend_timeout(self._max_disabling_wait_in_secs)
            handled = True

        elif event in [HOST_EVENT.INSTANCES_MOVED, HOST_EVENT.INSTANCES_STOPPED]:
            host_operation = event_data["host-operation"]
            success = not (host_operation.is_failed() or host_operation.is_timed_out())
            DLOG.info("Host %s %s, success=%s." % (self._host.name, event, success))
            if success:
                self.task.task_work_complete(
                    state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                )
                ...
                if self.force_pass:
                    self._host.update_failure_reason(host_operation.reason)
                    DLOG.info(
                        "Notify-Instances-Host-Disabling failed for %s, "
                        "force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
            ...
            if self._host.is_failed() and not self._host.is_component_failure():
                DLOG.info(
                    "Host %s is now failed, skipping host disabling "
                    "notifications to instances." % self._host.name
                )
                self.task.task_work_complete(
                    state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
...
class NotifyInstancesHostDisabledTaskWork(state_machine.StateTaskWork):
    ...
    def timeout(self):
        ...
        if not self._host.has_reason():
            self._host.update_failure_reason(
                "Moving instances from disabled host %s timed out." % self._host.name
            )
    ...
    def handle_event(self, event, event_data=None):
        ...
        if HOST_EVENT.INSTANCE_MOVED == event:
            ...
            success = not (host_operation.is_failed() or host_operation.is_timed_out())
            DLOG.info(
                "Host %s instances have moved, success=%s." % (self._host.name, success)
            )
            if success:
                self.task.task_work_complete(
                    state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
                ...
                if self.force_pass:
                    DLOG.info(
                        "Notify-Instances-Host-Disabled failed for %s, "
                        "force-passing." % self._host.name
                    )
                    self.task.task_work_complete(
                        state_machine.STATE_TASK_WORK_RESULT.SUCCESS, empty_reason
