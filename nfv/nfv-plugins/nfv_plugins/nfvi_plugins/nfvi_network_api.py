...
class NFVINetworkAPI(nfvi.api.v1.NFVINetworkAPI):
    ...
    def get_networks(self, future, paging, callback):
        ...
        try:
            ...
            future.work(
                neutron.get_networks, self._token, paging.page_limit, paging.next_page
            )
            future.result = yield
            ...
            DLOG.exception(
                "Caught exception while trying to get network list, error=%s." % e
            )
    ...
    def create_network(
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to create a network, error=%s." % e
                )
    ...
    def update_network(self, future, network_uuid, shared, callback):
        ...
        try:
            ...
            future.work(
                neutron.update_network, self._token, network_uuid, shared=shared
            )
            future.result = yield
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to update a network, error=%s." % e
                )
    ...
    def delete_network(self, future, network_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to delete a network, error=%s." % e
                )
    ...
    def get_network(self, future, network_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get a network, error=%s." % e
                )
    ...
    def get_subnets(self, future, paging, callback):
        ...
        try:
            ...
            future.work(
                neutron.get_subnets, self._token, paging.page_limit, paging.next_page
            )
            future.result = yield
            ...
            for subnet_data in subnet_data_list["subnets"]:
                ...
                subnet_obj = nfvi.objects.v1.Subnet(
                    subnet_data["id"],
                    subnet_data["name"],
            ...
            DLOG.exception(
                "Caught exception while trying to get subnet list, error=%s." % e
            )
    ...
    def create_subnet(
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to create a subnet, error=%s." % e
                )
    ...
    def update_subnet(
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to update a subnet, error=%s." % e
                )
    ...
    def delete_subnet(self, future, subnet_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to delete a subnet, error=%s." % e
                )
    ...
    def get_subnet(self, future, subnet_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get a subnet, error=%s." % e
                )
    ...
    def delete_host_services(
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if neutron.lookup_extension(
                    neutron.EXTENSION_NAMES.AGENT, self._neutron_extensions
                ):
                    response["reason"] = "failed to delete neutron services"

                    future.work(neutron.delete_network_agents, self._token, host_name)
                    try:
                        future.result = yield

                        if not future.result.is_complete():
                            DLOG.error(
                                "Neutron delete-host-services failed, "
            ...
            DLOG.exception(
                ...
                "neutron openstack services, error=%s." % (host_name, e)
            )
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if neutron.lookup_extension(
                    ...
                    future.work(neutron.enable_network_agents, self._token, host_name)

                    future.result = yield

                    if not future.result.is_complete():
                        DLOG.error(
                            "Neutron enable-host-services failed, "
                    ...
                    if not future.result.data:
                        DLOG.error(
                            "Neutron enable-host-services (agents) failed, "
                            "operation did not complete, host_uuid=%s, "
                            "host_name=%s." % (host_uuid, host_name)
                        )
                        return
    ...
    def get_network_agents(self, future, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(neutron.get_network_agents, self._token)

                future.result = yield
                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron get-network-agents failed, operation did not complete"
                    )
                    return
                else:
    ...
    def get_agent_routers(self, future, agent_id, callback):
        ...
        try:
            ...
            if self._token is None or self._token.is_expired():
                ...
                if not future.result.is_complete() or future.result.data is None:
                    DLOG.error(
                        "OpenStack get-token did not complete, agent_id=%s." % agent_id
                    )
                    return
            ...
            if neutron.lookup_extension(
                ...
                future.work(neutron.get_agent_routers, self._token, agent_id)

                future.result = yield

                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron get-agent-routers failed, "
                        "operation did not complete, agent_id=%s" % agent_id
                    )
                    return
                else:
                    response["result-data"] = future.result.data
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get agent routers, error=%s." % e
                )

        except Exception as e:
            DLOG.exception(
                "Caught exception while trying to get %s "
                "neutron agent routers, error=%s." % (agent_id, e)
            )
    ...
    def get_dhcp_agent_networks(self, future, agent_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(neutron.get_dhcp_agent_networks, self._token, agent_id)

                future.result = yield

                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron get-dhcp-agent-networks failed, "
                        "operation did not complete, agent_id=%s" % agent_id
                    )
                    return
                else:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get dhcp networks, error=%s." % e
                )

        except Exception as e:
            DLOG.exception(
                "Caught exception while trying to get %s "
                "neutron dhcp networks, error=%s." % (agent_id, e)
            )
    ...
    def get_router_ports(self, future, router_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(neutron.get_router_ports, self._token, router_id)

                future.result = yield

                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron get-router-ports failed, "
                        "operation did not complete, router_id=%s" % router_id
                    )
                    return
                else:
                    response["result-data"] = future.result.data
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get router ports, error=%s." % e
                )

        except Exception as e:
            DLOG.exception(
                "Caught exception while trying to get %s "
                "router ports, error=%s." % (router_id, e)
            )
    ...
    def add_router_to_agent(self, future, agent_id, router_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(
                    neutron.add_router_to_agent, self._token, agent_id, router_id
                )
                ...
                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron add-router-to-agent failed, "
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to add "
            ...
            DLOG.exception(
                "Caught exception while trying to add "
                "router_id=%s to agent_id=%s, error=%s." % (router_id, agent_id, e)
            )
    ...
    def add_network_to_dhcp_agent(self, future, agent_id, network_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(
                    neutron.add_network_to_dhcp_agent, self._token, agent_id, network_id
                )
                ...
                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron add-network-to-dhcp-agent failed, "
                        "operation did not complete, agent_id=%s "
            ...
            DLOG.exception(
                "Caught exception while trying to add "
                "network_id=%s to agent_id=%s, error=%s." % (network_id, agent_id, e)
            )
    ...
    def remove_router_from_agent(self, future, agent_id, router_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(
                    neutron.remove_router_from_agent, self._token, agent_id, router_id
                )
                ...
                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron remove-router-from-agent failed, "
                        "operation did not complete, agent_id=%s "
                        "router_id=%s" % (agent_id, router_id)
                    )
                    return
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to remove "
            ...
            DLOG.exception(
                "Caught exception while trying to remove "
                "router_id=%s from agent_id=%s, error=%s." % (router_id, agent_id, e)
            )
    ...
    def remove_network_from_dhcp_agent(self, future, agent_id, network_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(
                    neutron.remove_network_from_dhcp_agent,
                    self._token,
                    agent_id,
                    network_id,
                )
                ...
                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron remove-network-from-dhcp-agent failed, "
                        "operation did not complete, agent_id=%s "
                        "network_id=%s" % (agent_id, network_id)
                    )
                    return
            ...
            DLOG.exception(
                "Caught exception while trying to remove "
                "network_id=%s from agent_id=%s, error=%s." % (network_id, agent_id, e)
            )
    ...
    def get_physical_network(self, future, network_id, callback):
        ...
        try:
            ...
            if neutron.lookup_extension(
                ...
                future.work(neutron.get_physical_network, self._token, network_id)

                future.result = yield

                if not future.result.is_complete():
                    DLOG.error(
                        "Neutron get-physical-network failed, "
                        "operation did not complete, network_id=%s" % network_id
                    )
                    return
                else:
                    response["result-data"] = future.result.data
            ...
            DLOG.exception(
                "Caught exception while trying to get %s "
                "physical network, error=%s." % (network_id, e)
            )
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if neutron.lookup_extension(
                    ...
                    if not future.result.is_complete():
                        DLOG.error(
                            "Neutron query-host-services failed, "
                            "operation did not complete, host_uuid=%s, "
                            "host_name=%s." % (host_uuid, host_name)
                        )
                        return
        ...
        try:
            ...
            if self._host_supports_neutron(host_personality):
                ...
                if neutron.lookup_extension(
                    ...
                    future.work(neutron.disable_network_agents, self._token, host_name)

                    future.result = yield

                    if not future.result.is_complete():
                        DLOG.error(
                            "Neutron disable-host-services failed, "
                            "operation did not complete, host_uuid=%s, "
                            "host_name=%s." % (host_uuid, host_name)
                        )
                        return
                    ...
                    if not future.result.data:
                        DLOG.error(
                            "Neutron disable-host-services (agents) failed, "
                            "operation did not complete, host_uuid=%s, "
                            "host_name=%s." % (host_uuid, host_name)
                        )
                        return
