...
class NFVIImageAPI(nfvi.api.v1.NFVIImageAPI):
    ...
    def get_images(self, future, paging, callback):
        ...
        try:
            ...
            future.work(
                glance.get_images, self._token, paging.page_limit, paging.next_page
            )
            future.result = yield
            ...
            for image_data in image_data_list["images"]:
                ...
                if sw_wrs_auto_recovery is not None:
                    if "false" == sw_wrs_auto_recovery.lower():
                        ...
                        raise AttributeError(
                            "sw_wrs_auto_recovery is %s, "
                            "expecting 'true' or 'false'" % sw_wrs_auto_recovery
                        )
                ...
                image_obj = nfvi_objs.Image(
                    ...
                    avail_status,
                    action,
                    image_data["container_format"],
                    image_data["disk_format"],
                    image_data["min_disk"],
                    image_data["min_ram"],
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get image list, error=%s." % e
                )
    ...
    def create_image(
        ...
        try:
            ...
            future.work(
                ...
                image_name,
                image_description,
                image_attributes.container_format,
            ...
            if image_data_url.startswith("file://"):
                ...
                future.work(
                    glance.upload_image_data_by_file,
                    self._token,
                    image_data["id"],
                    image_file,
                )
                ...
                future.work(
                    glance.upload_image_data_by_url,
                    self._token,
                    image_data["id"],
                    image_data_url,
                )
            ...
            if sw_wrs_auto_recovery is not None:
                if "false" == sw_wrs_auto_recovery.lower():
                    properties[nfvi_objs.IMAGE_PROPERTY.INSTANCE_AUTO_RECOVERY] = False

                elif "true" == sw_wrs_auto_recovery.lower():
                    properties[nfvi_objs.IMAGE_PROPERTY.INSTANCE_AUTO_RECOVERY] = True
            ...
            properties[nfvi_objs.IMAGE_PROPERTY.INSTANCE_AUTO_RECOVERY] = (
                image_data.get(nfvi_objs.IMAGE_PROPERTY.INSTANCE_AUTO_RECOVERY, None)
            )

            properties[nfvi_objs.IMAGE_PROPERTY.LIVE_MIGRATION_TIMEOUT] = (
                image_data.get(nfvi_objs.IMAGE_PROPERTY.LIVE_MIGRATION_TIMEOUT, None)
            )
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to create an image, error=%s." % e
                )
    ...
    def delete_image(self, future, image_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to delete an image, error=%s." % e
                )
    ...
    def get_image(self, future, image_uuid, callback):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to get an image, error=%s." % e
                )
