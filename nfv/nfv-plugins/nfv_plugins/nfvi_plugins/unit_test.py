...
def glance_unit_tests(token, test_config):
    ...
    image = glance.create_image(
        token, "image-test", "", "bare", "qcow2", 1, 512, "public", False, None
    )
    print("Created image: %s" % image)

    glance.upload_image_data_by_url(token, image["id"], test_config["image_url"])

    image = glance.get_image(token, image["id"])
    print("Image details: %s" % image)

    image = glance.create_image(
        token, "image-test2", "", "bare", "qcow2", 2, 1024, "private", False, None
    )
    print("Created image: %s" % image)

    glance.upload_image_data_by_file(token, image["id"], test_config["image_file"])
...
def neutron_unit_tests(token, test_config):
    ...
    network = neutron.create_network(
        token, "network-test", "vlan", 450, "physnet0", False
    )
    print("Created network: %s" % network)

    network = neutron.update_network(
        token, network["network"]["id"], neutron.NETWORK_ADMIN_STATE.DOWN, True
    )
    ...
    subnet = neutron.update_subnet(
        token, subnet["subnet"]["id"], delete_gateway=True, dhcp_enabled=False
    )
...
def nova_unit_tests(token, test_config):
    ...
    images = glance.get_images(token)
    image = next(image for image in images["images"] if image["name"] == "image-test")
    ...
    status = None
    nova.rebuild_server(
        token, server["server"]["id"], server["server"]["name"], image["id"]
    )
    while "ACTIVE" != status:
        server = nova.get_server(token, server["server"]["id"])
        status = server["server"]["status"]
...
def heat_unit_tests(token, test_config):
    ...
    heat.delete_stack(token, stack["stack"]["stack_name"], stack["stack"]["id"])
    print("Stack deleted")
...
def guest_unit_tests(token, test_config):
    ...
    response = guest.host_services_create(
        token, test_config["host_uuid"], test_config["host_name"]
    )
    print("host_services_create response: %s" % response)

    response = guest.host_services_enable(
        token, test_config["host_uuid"], test_config["host_name"]
    )
    print("host_services_enable response: %s" % response)

    response = guest.host_services_disable(
        token, test_config["host_uuid"], test_config["host_name"]
    )
    print("host_services_disable response: %s" % response)

    response = guest.host_services_query(
        token, test_config["host_uuid"], test_config["host_name"]
    )
    ...
    response = guest.guest_services_create(
        token, test_config["instance_uuid"], test_config["host_name"], services
    )
    print("guest_services_create response: %s" % response)

    services = {"heartbeat": "enabled", "massage": "disabled"}
    response = guest.guest_services_set(
        token, test_config["instance_uuid"], test_config["host_name"], services
    )
