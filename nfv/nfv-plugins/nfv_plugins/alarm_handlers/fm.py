...
# todo(abailey): refactor each orchestration into its own mapping

_fm_kube_rootca_update_alarm_id_mapping = dict(
    [
        (
            alarm_objects_v1.ALARM_TYPE.KUBE_ROOTCA_UPDATE_AUTO_APPLY_INPROGRESS,
            fm_constants.FM_ALARM_ID_KUBE_ROOTCA_UPDATE_AUTO_APPLY_INPROGRESS,
        ),
        (
            alarm_objects_v1.ALARM_TYPE.KUBE_ROOTCA_UPDATE_AUTO_APPLY_ABORTING,
            fm_constants.FM_ALARM_ID_KUBE_ROOTCA_UPDATE_AUTO_APPLY_ABORTING,
        ),
        (
            alarm_objects_v1.ALARM_TYPE.KUBE_ROOTCA_UPDATE_AUTO_APPLY_FAILED,
            fm_constants.FM_ALARM_ID_KUBE_ROOTCA_UPDATE_AUTO_APPLY_FAILED,
        ),
    ]
)
...
_fm_alarm_severity_mapping = dict(
    [
        (alarm_objects_v1.ALARM_SEVERITY.CLEARED, fm_constants.FM_ALARM_SEVERITY_CLEAR),
        (
            alarm_objects_v1.ALARM_SEVERITY.WARNING,
            fm_constants.FM_ALARM_SEVERITY_WARNING,
        ),
        (alarm_objects_v1.ALARM_SEVERITY.MINOR, fm_constants.FM_ALARM_SEVERITY_MINOR),
        (alarm_objects_v1.ALARM_SEVERITY.MAJOR, fm_constants.FM_ALARM_SEVERITY_MAJOR),
        (
...
class FaultManagement(alarm_handlers_v1.AlarmHandler):
    ...
    def openstack_token(self):
        if self._openstack_token is None or self._openstack_token.is_expired():
    ...
    def _format_alarm(self, alarm_data):
        ...
        if fm_alarm_id is not None:
            ...
            fault = fm_api.Fault(
                ...
                alarm_data.entity,
                fm_severity,
                alarm_data.specific_problem_text,
                fm_alarm_type,
                fm_probable_cause,
    ...
    def raise_alarm(self, alarm_uuid, alarm_data):
        ...
        if fault is not None:
            ...
            if fm_uuid is None:
                DLOG.error(
                    "Failed to raise alarm, uuid=%s, fm_uuid=%s."
                    % (alarm_uuid, fm_uuid)
                )
            else:
                DLOG.info("Raised alarm, uuid=%s, fm_uuid=%s." % (alarm_uuid, fm_uuid))
        else:
    ...
    def _clear_openstack_alarm(self, fm_uuid):
        ...
        try:
            ...
            if httplib.UNAUTHORIZED == e.http_status_code:
                ...
                DLOG.exception(
                    "Caught exception while trying to clear alarm %s, "
                    "error=%s." % (fm_uuid, e)
                )
        except Exception as e:
            DLOG.exception(
    ...
    def _audit_openstack_alarms(self):
        ...
        for fm_alarm in fm_alarms:
            for alarm_uuid, (alarm_data, fm_uuid) in list(
                self._openstack_alarm_db.items()
            ):
                if fm_uuid == fm_alarm["uuid"]:
                    break
            else:
                DLOG.info(
                    "Clear stale alarm, fm_uuid=%s, fm_alarm_id=%s, "
            ...
            for fm_fault in fm_faults:
                for alarm_uuid, (alarm_data, fm_uuid) in list(
                    ...
                    self._fm_api.clear_fault(
                        fm_fault.alarm_id, fm_fault.entity_instance_id
                    )
    ...
    def initialize(self, config_file):
        ...
        self._openstack_fm_endpoint_disabled = (
            config.CONF["fm"]["endpoint_disabled"] in DISABLED_LIST
        )
        ...
        self._fault_management_pod_disabled = (
            config.CONF["openstack"].get("fault_management_pod_disabled", "True")
            in DISABLED_LIST
        )
