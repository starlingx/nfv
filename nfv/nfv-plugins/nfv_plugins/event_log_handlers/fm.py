...
_fm_event_type_mapping = dict(
    [
        (event_log_objects_v1.EVENT_TYPE.STATE_EVENT, fm_constants.FM_ALARM_TYPE_4),
        (event_log_objects_v1.EVENT_TYPE.ACTION_EVENT, fm_constants.FM_ALARM_TYPE_4),
        (
...
class EventLogManagement(event_log_handlers_v1.EventLogHandler):
    ...
    def log(self, log_data):
        ...
        if fault is not None:
            ...
            if "instance" in log_data.entity_type and (
            ...
            if fm_uuid is None:
                DLOG.error("Failed to generate customer log, fm_uuid=%s." % fm_uuid)
            else:
