...
class FileStorage(event_log_handlers_v1.EventLogHandler):
    ...
    def _write_log_data(self, log_data):
        ...
        with codecs.open(config.CONF["File-Storage"]["file"], "a", "utf-8") as f:
            ...
            print("%-24s = %s" % ("event-id", log_data.event_id), file=f)
            print("%-24s = %s" % ("event-type", log_data.event_type), file=f)
            print("%-24s = %s" % ("event-context", log_data.event_context), file=f)
            print("%-24s = %s" % ("importance", log_data.importance), file=f)
            print("%-24s = %s" % ("entity", log_data.entity), file=f)
            print("%-24s = %s" % ("reason_text", log_data.reason_text), file=f)
            print("%-24s = %s" % ("additional_text", log_data.additional_text), file=f)

            if log_data.state_data is not None:
                print(
                    "%-24s = %s" % ("state-data.state", log_data.state_data.state),
                    file=f,
                )
            ...
            if log_data.threshold_data is not None:
                print(
                    ...
                    % (
                        ...
                        log_data.threshold_data.threshold_value,
                    ),
                    file=f,
                )
                print(
                    ...
                    % (
                        ...
                        log_data.threshold_data.observed_value,
                    ),
                    file=f,
                )

            if log_data.timestamp is not None:
                print("%-24s = %s" % ("timestamp", log_data.timestamp), file=f)
