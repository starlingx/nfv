...
def add_list_arg(some_cmd, some_arg, some_list):
    ...
    """
    default = some_list[0]
    some_cmd.add_argument(
        some_arg, default=default, choices=some_list, help="defaults to " + default
    )
...
def setup_abort_cmd(parser):
    ...
    """
    abort_cmd = parser.add_parser("abort", help="Abort a strategy")
    abort_cmd.set_defaults(cmd="abort")
    abort_cmd.add_argument("--stage-id", help="stage identifier to abort")
    abort_cmd.add_argument(
        "--yes", action="store_true", help="automatically confirm the action"
    )
    return abort_cmd
...
def setup_apply_cmd(parser):
    ...
    """
    apply_cmd = parser.add_parser("apply", help="Apply a strategy")
    apply_cmd.set_defaults(cmd="apply")
    apply_cmd.add_argument("--stage-id", default=None, help="stage identifier to apply")
...
def setup_delete_cmd(parser):
    ...
    delete_cmd.add_argument("--force", action="store_true", help=argparse.SUPPRESS)
    return delete_cmd
...
def setup_show_cmd(parser):
    ...
    show_cmd.add_argument(
        "--active", action="store_true", help="show currently active strategy step"
    )
    show_cmd.add_argument(
        "--error-details",
        action="store_true",
...
def setup_fw_update_parser(commands):
    ...
    cmd_parser = commands.add_parser(cmd_area, help="Firmware Update Strategy")
    cmd_parser.set_defaults(cmd_area=cmd_area)

    sub_cmds = cmd_parser.add_subparsers(title="Firmware Update Commands", metavar="")
...
def setup_kube_rootca_update_parser(commands):
    ...
    cmd_parser = commands.add_parser(cmd_area, help="Kubernetes RootCA Update Strategy")
    cmd_parser.set_defaults(cmd_area=cmd_area)

    sub_cmds = cmd_parser.add_subparsers(
        title="Kubernetes RootCA Update Commands", metavar=""
    )
    sub_cmds.required = True
...
def setup_kube_upgrade_parser(commands):
    ...
    cmd_parser = commands.add_parser(cmd_area, help="Kubernetes Upgrade Strategy")
    cmd_parser.set_defaults(cmd_area=cmd_area)
...
def setup_system_config_update_parser(commands):
    ...
    cmd_parser = commands.add_parser(cmd_area, help="system config update Strategy")
    cmd_parser.set_defaults(cmd_area=cmd_area)

    sub_cmds = cmd_parser.add_subparsers(
        title="Sytem Config Update Commands", metavar=""
    )
    sub_cmds.required = True
...
def setup_sw_deploy_parser(commands):
    ...
    cmd_parser = commands.add_parser(cmd_area, help="Software Deploy Strategy")
    cmd_parser.set_defaults(cmd_area=cmd_area)

    sub_cmds = cmd_parser.add_subparsers(
        title="Software Deploy Commands", metavar="[create, delete, apply, abort, show]"
    )
    sub_cmds.required = True
    ...
    create_strategy_cmd = setup_create_cmd(
        sub_cmds,
        [sw_update.APPLY_TYPE_SERIAL, sw_update.APPLY_TYPE_IGNORE],
        [
...
def _is_service_impacting_command(command):
    ...
    service_impacting_commands = {
        f"{cmd} {action}" for cmd in base_commands for action in actions
    }
...
def process_main(argv=sys.argv[1:]):  # pylint: disable=dangerous-default-value
    ...
    try:
        ...
        if args.os_project_domain_name is None:
            args.os_project_domain_name = os.environ.get(
        ...
        if "create" == args.cmd:
            ...
            raise ValueError(
                "Unknown command, %s, given for %s" % (args.cmd, args.cmd_area)
            )
