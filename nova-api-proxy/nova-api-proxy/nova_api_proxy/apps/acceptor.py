...
class APIController(Middleware):
    ...
    def __call__(self, req):
        ...
        if req.environ["REQUEST_METHOD"] == "POST" and self._is_nfvi_request(req):
    ...
    def _is_nfvi_request(self, request):
        ...
        for action in self._actions:
            if action in list(data):
                ...
                LOG.info(
                    'Forward to NFV "%s %s", action: (%s), val:(%s)'
                    % (
    ...
    def _log_message(self, environ):
        ...
        LOG.info(
            "%s request issued by user (%s) tenant (%s) remote address "
            '(%s) "%s %s"'
            % (
    ...
    def _print_data(self, obj):
        if isinstance(obj, dict):
            ...
            for v in obj:  # pylint: disable=invalid-name
                if hasattr(v, "__iter__"):
                    self._print_data(v)
                else:
                    LOG.info("%s" % v)
