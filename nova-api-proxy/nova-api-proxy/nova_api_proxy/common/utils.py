...
def set_request_forward_environ(req, remote_host, remote_port):
    ...
    # Set standard forwarding headers for oslo_middleware http_proxy_to_wsgi
    if "HTTP_X_FORWARDED_HOST" not in req.environ:
        req.environ["HTTP_X_FORWARDED_HOST"] = req.environ.get("HTTP_HOST", "")
    if "HTTP_X_FORWARDED_PROTO" not in req.environ:
        req.environ["HTTP_X_FORWARDED_PROTO"] = req.environ["wsgi.url_scheme"]
