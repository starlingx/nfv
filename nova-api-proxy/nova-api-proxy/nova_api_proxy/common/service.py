...
server_opts = [
    ...
    cfg.IntOpt(
        "pool_size", default=1000, help="Size of the pool of greenthreads used by wsgi"
    ),
    cfg.IntOpt(
        "max_header_line",
        default=16384,
...
class Server(object):
    ...
    def _setup_ssl(self):
        LOG.info("%(name)s setup ssl" % self.__dict__)  # noqa: H501
        try:
            ...
            LOG.error(
                "Failed to start %(name)s on %(host)s :%(port)s with SSL"  # noqa: H501
    ...
    def start(self):
        ...
        if hasattr(socket, "TCP_KEEPIDLE"):
            self._socket.setsockopt(
                socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, CONF.tcp_keepidle
            )
...
class Middleware(Application):
    ...
    def factory(cls, global_config, **local_config):
        ...
        def _factory(app):
            return cls(  # pylint: disable=too-many-function-args
                app, global_config, **local_config
            )
    ...
    def process_request(self, req):  # pylint: disable=unused-argument
    ...
    def __call__(self, req):
        response = self.process_request(req)  # pylint: disable=assignment-from-none
        if response:
            return response
